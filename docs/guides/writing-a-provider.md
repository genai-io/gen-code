...
[`internal/llm/sdk`](../packages/2-feature/llm.md).
...
If it is, adding it to San is one entry in `entries` in
`internal/llm/sdk/vendors.go`:
...
Plus a row in `displays` for the provider's name and sort order, and a constant
...
`internal/llm/sdk/auth.go`: the flow belongs to the SDK, the storage to San.

## Tests

`internal/llm/sdk/sdk_test.go` drives the seam against stub endpoints —
what reached the wire, and what came back. `live_test.go` runs one real turn per
configured vendor and is opt-in:

```sh
SAN_SDK_LIVE=1 go test ./internal/llm/sdk/ -run TestLive -v
```
